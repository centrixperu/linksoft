...
var fa_frm_tab = ['Ficha', 'Características', 'Imágenes'];
...
function fnc_child_objects_load_functions() {
	...
	$(Obj_Col(fc_frm_cb, 'lin', 'select')).change(function() {
		...
		if (lc_ccod_opfa !== '') {
			var pc_url = '/home/lq_usp_al_ct_slin_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_lin=' + lc_ccod_opfa + '&ic_cest=' + fc_ct_cest_Activo + '&ic_load_BD=';
	...
	// Impuestos
	$(
		Obj_Col(fc_frm_cb, 'imp_igv') + ', ' +
		Obj_Col(fc_frm_cb, 'imp_isc') + ', ' +
		Obj_Col(fc_frm_cb, 'imp_per')
	).change(function() {
		fnc_select_imp_change(this);
	});
	
	$(
		Obj_Col(fc_frm_cb, 'bigv') + ', ' +
		Obj_Col(fc_frm_cb, 'bisc') + ', ' +
		Obj_Col(fc_frm_cb, 'bper')
	).on('change', function(e) {
		var ac_object = fnc_GET_Obj_Attributes(this);
		var pc_data_col = ac_object.col;
		pc_data_col = pc_data_col.substring(1, pc_data_col.length);
		
		if ($(this).is(':checked'))
		{
			fnc_checkbox_set_value(fc_frm_cb, 'bexonerado', false);
			
			fnc_SET_Obj_Attributes(fc_frm_cb, 'imp_' + pc_data_col, [], [], true);
			$('.row_imp_' + pc_data_col).removeClass('display_none');
		}
		else
		{
			$(Obj_Col(fc_frm_cb, 'imp_' + pc_data_col)).val('').trigger('change');
			fnc_SET_Obj_Attributes(fc_frm_cb, 'imp_' + pc_data_col, [], [], false);
			$('.row_imp_' + pc_data_col).addClass('display_none');
		}
	});

	// Exonerado
	$(Obj_Col(fc_frm_cb, 'bexonerado')).on('change', function(e) {
		if ($(this).is(':checked'))
		{
	...
	// Series
	$(Obj_Col(fc_frm_cb, 'bserie')).on('change', function(e) {
		if ($(this).is(':checked'))
		{
	...
	// Lote
	$(Obj_Col(fc_frm_cb, 'blote')).on('change', function(e) {
		if ($(this).is(':checked'))
		{
...
{
	f_eject_ajax(
				'/home/lq_usp_al_ct_it_dt_um_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_it=' + '&ic_ccod_um=' + '&in_row=0' + '&ic_load_BD=load_BD'
				);

	fnc_table_reload(
					'grid_lista',
					'/home/lq_usp_al_ct_it_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ctipo_it=' + 'B' + '&ic_cest=' + '&ic_load_BD=load_BD',
					'grid_lista'
					);
...
function fnc_child_registrar()
{
	...
	var pa_RowData_cab = [
							...
							'B',
							$(Obj_Col(fc_frm_cb, 'tip_prod')).find(":selected").val(),
							...
							($(Obj_Col(fc_frm_cb, 'bserie')).is(':checked') ? '1' : '0'),
							($(Obj_Col(fc_frm_cb, 'blote')).is(':checked') ? '1' : '0'),
							$(Obj_Col(fc_frm_cb, 'cstock_min')).val(),
							$(Obj_Col(fc_frm_cb, 'cstock_max')).val(),
							($(Obj_Col(fc_frm_cb, 'bexonerado')).is(':checked') ? '1' : '0'),
							($(Obj_Col(fc_frm_cb, 'bigv')).is(':checked') ? '1' : '0'),
							($(Obj_Col(fc_frm_cb, 'bigv')).is(':checked') ? $(Obj_Col(fc_frm_cb, 'ccod_imp_igv')).val() : ''),
							($(Obj_Col(fc_frm_cb, 'bisc')).is(':checked') ? '1' : '0'),
							($(Obj_Col(fc_frm_cb, 'bisc')).is(':checked') ? $(Obj_Col(fc_frm_cb, 'ccod_imp_isc')).val() : ''),
							($(Obj_Col(fc_frm_cb, 'bper')).is(':checked') ? '1' : '0'),
							($(Obj_Col(fc_frm_cb, 'bper')).is(':checked') ? $(Obj_Col(fc_frm_cb, 'ccod_imp_per')).val() : '')
						];
	...
	var pa_data = [pc_RowData_cab, pc_RowData_um];
	return [pa_data];
};
...
function fnc_child_eliminar() {
	return ['Eliminar', [fnc_Get_File_Values(['d', gc_ccod_emp, fr_row_data_cb.ccod_it]), ''], [], []];
};
...
function fnc_grid_lista_setear_campos_child_post(row_data) {
	...
	// IGV
	if ($(Obj_Col(fc_frm_cb, 'ntasa_imp_igv')).val() !== '' && parseInt($(Obj_Col(fc_frm_cb, 'ntasa_imp_igv')).val()) !== 0)
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_igv')).val(fnc_formatear_importe($(Obj_Col(fc_frm_cb, 'ntasa_imp_igv')).val()).toString() + ' %');
	}
	else
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_igv')).val('');
	}
	
	// ISC
	if ($(Obj_Col(fc_frm_cb, 'ntasa_imp_isc')).val() !== '' && parseInt($(Obj_Col(fc_frm_cb, 'ntasa_imp_isc')).val()) !== 0)
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_isc')).val(fnc_formatear_importe($(Obj_Col(fc_frm_cb, 'ntasa_imp_isc')).val()).toString() + ' %');
	}
	else
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_isc')).val('');
	}

	// Percepcion
	if ($(Obj_Col(fc_frm_cb, 'ntasa_imp_per')).val() !== '' && parseInt($(Obj_Col(fc_frm_cb, 'ntasa_imp_per')).val()) !== 0)
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_per')).val(fnc_formatear_importe($(Obj_Col(fc_frm_cb, 'ntasa_imp_per')).val()).toString() + ' %');
	}
	else
	{
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_per')).val('');
	}
...
function fnc_select_imp_change(io_this) {
	...
	if ($(io_this).find(":selected").val() !== '') {
		var pc_url = '/home/lq_usp_co_ct_imp_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_imp=' + $(io_this).find(":selected").val() + '&ic_cest=' + '&ic_load_BD=';
		...
		$(Obj_Col(fc_frm_cb, 'ntasa_imp_' + data_col)).val('');
	}
};
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::																																					::
::																ROWS: Otras presentaciones															::
::																																					::
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
*/

var fc_div_head_row_um = '#title_row_um';
var fc_div_row_um = 'row_um_';
var fn_id_row_um = 0;

function fnc_row_um_nuevo() {
	fn_id_row_um = 0;
	$("div[data-row*='" + fc_div_row_um + "']").remove();
	fnc_row_remove(fc_div_row_um, fc_div_head_row_um);
};

function fnc_add_row_um() {
	...
	$(fc_div_head_row_um).removeClass('display_none');
	fnc_add_div_row_um();

	$(Obj_Col(fc_frm_cb, 'ccod_um_eq_' + fn_id_row_um)).val(pc_ccod_um_eq);
	f_obj_validar_vacio(fc_frm_cb, 'ncant_eq_' + fn_id_row_um, 'input', '');

	f_obj_validar_vacio(fc_frm_cb, 'um_' + fn_id_row_um, 'select', '');
};

function fnc_add_div_row_um () {
	fn_id_row_um = fn_id_row_um + 1;
	fo_div_row = $('<div data-row="' + fc_div_row_um + String(fn_id_row_um) + '" class="row ft_2_data-row" />');
	
	fo_div_row_object_1 = $('<div class="col s3 display_none"><input type="text" class="lks" data-col="ccod_um_' + fn_id_row_um + '" data-clear="n" data-hb_v="n" data-hb_n="s" data-hb_e="s" required /></div>');
	fo_div_row_object_2 = $('<div class="col s4"><select class="select2 s2" data-col="um_' + fn_id_row_um + '" data-dsc="Unidad de Medida" data-allow-clear="false" data-hb_v="n" data-hb_n="s" data-hb_e="s" data-i_ccod required></select></div>');

	if (jc_est_form === 'n' || jc_est_form === 'e') {pc_string = "<div class='col s3'>";}
	else if (jc_est_form === 'v') {pc_string = "<div class='col s4' data-col_before='s3' data-col_after='s4'>";}
	fo_div_row_object_3 = $(pc_string + '<input type="text" class="lks text_align_right f_sin" data-col="ncant_eq_' + fn_id_row_um + '" data-dsc="Cantidad equivalente" data-clear="n" data-hb_v="n" data-hb_n="s" data-hb_e="s" required /></div>');
	
	fo_div_row_object_4 = $('<div class="col s4"><input type="text" class="lks" data-col="ccod_um_eq_' + fn_id_row_um + '" readonly /></div>');
	
	fnc_div_row_fo_button_remove('' ,'_lnk_row_remove_um_' + String(fn_id_row_um), fc_div_row_um, fc_div_head_row_um, '');
	...
	$("div[data-rows='um']").append(fo_div_row);
	
	// Caracteristicas del input
	fnc_inputs_cargar_caracteristicas('form_main', 'ncant_eq_' + fn_id_row_um);
	fnc_inputs_cargar_caracteristicas('form_main', 'ccod_um_eq_' + fn_id_row_um);
	
	// Caracteristicas del select
	fnc_selects_cargar_caracteristicas(fc_frm_cb, 'um_' + fn_id_row_um);
	f_select_input(fc_frm_cb, 'hb', $(Obj_Col(fc_frm_cb, 'um_' + fn_id_row_um)), 'n');
	//f_load_select_ajax(fc_frm_cb, 'um_' + fn_id_row_um, 'ccod_um', 'cdsc_um', '/home/lq_usp_al_ct_um_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ctipo_um=B' + '&ic_cest=' + fc_ct_cest_Activo + '&ic_load_BD=', false, pc_ccod_um);
	f_load_select_ajax(fc_frm_cb, 'um_' + fn_id_row_um, 'ccod_um', 'cdsc_um', '/home/lq_usp_al_ct_um_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ctipo_um=B' + '&ic_cest=' + fc_ct_cest_Activo + '&ic_load_BD=', false, '');

};

function fnc_add_rows_um() {
	var pc_ccod_um = '';
	var pn_ncant_eq = 0;
	var pc_ccod_um_eq = '';
	// Variables Ajax
	var pa_columns = ['ccod_um', 'ncant_eq', 'ccod_um_eq'];
	var pc_url = '/home/lq_usp_al_ct_it_dt_um_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_it=' + fr_row_data_cb.ccod_it + '&ic_ccod_um=' + '&in_row=2' + '&ic_load_BD=';
	...
	for (var i = 0; i < pa_results.length; i++) {
		pc_ccod_um = pa_results[i].ccod_um;
		pn_ncant_eq = pa_results[i].ncant_eq;
		pc_ccod_um_eq = pa_results[i].ccod_um_eq;

		fnc_add_div_row_um();
	
		$(Obj_Col(fc_frm_cb, 'ccod_um_' + fn_id_row_um)).val(pc_ccod_um);
		$(Obj_Col(fc_frm_cb, 'ncant_eq_' + fn_id_row_um)).val(pn_ncant_eq).trigger('change');
		$(Obj_Col(fc_frm_cb, 'ccod_um_eq_' + fn_id_row_um)).val(pc_ccod_um_eq);

		$(Obj_Col(fc_frm_cb, 'um_' + String(fn_id_row_um))).val(pc_ccod_um).trigger('change');
	}
...
// Mejorar identificando ambos procesos y luego eliminar todo este bloque de IMPUESTOS que ya no se usa
/*
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::																																					::
::																	ROWS: Impuestos																	::
...
function fnc_add_div_row_imp () {
	...
	f_load_select_ajax(fc_frm_cb, 'imp_' + fn_id_row_imp, 'ccod_imp', 'cdsc_imp', '/home/lq_usp_co_ct_imp_list?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_imp=' + '&ic_cest=' + fc_ct_cest_Activo + '&ic_load_BD=', false, '');
