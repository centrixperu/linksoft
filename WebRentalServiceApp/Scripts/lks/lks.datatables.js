...
function fnc_si_usu_ctl_conf (pi_cctrl, pi_grid_option, pi_grid_attributes, ic_data_zi = '', ib_table_fixed = false, ib_required = false)
{
	...
	var pa_results = fnc_Array_Filtrar_por_columnas(ma_usp_Get_si_usu_ctl_conf, ['cform', 'cctrl'], [(fb_grid_source_generic === true ? 'generic' : fc_cfrm), pi_cctrl]);
	pa_results = fnc_Array_sortByKeyAsc(pa_results, 'nord');
	/*
	var pc_url = '/Home/lq_usp_Get_si_usu_ctl_conf?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_usu=' + gc_ccod_usu + '&ic_cform=' + (fb_grid_source_generic === true ? 'generic' : fc_cfrm) + '&pi_cctrl=' + pi_cctrl + '&pi_cft_group_cdsc=' + '&ic_load_BD=';
	...
	*/
	...
	/*
	pc_url = '/Home/lq_usp_Get_si_usu_ctl_conf?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_usu=' + gc_ccod_usu + '&ic_cform=' + (fb_grid_source_generic === true ? 'generic' : fc_cfrm) + '&pi_cctrl=' + pi_cctrl + '&pi_cft_group_cdsc=SI' + '&ic_load_BD=';
...
)
{
	...
	var cad_ = '';
	// Modal begin
	cad_ += "<div id='mdl_" + pi_grid_id + "' class='modal modal-fixed-footer_ft_1 modal_ft_search " + ic_modal_class + "' data-button_close='" + pi_grid_var_button_close + "'>";
	...
	// Content begin
	// cad_ += "	<div class='modal-content st_lks height_-134px' data-col='scroll_mdl_" + pi_grid_id + "' data-col_2='div_width_-5px'>";
	cad_ += "	<div class='modal-content st_lks height_-119px'>";
	
	cad_ += "		<div class='col s12 sub_container full_height'>";
	cad_ += "			<div class='col s12 container_table footer_fixed'>";
	cad_ += "				<section class='ft_1'>";
	cad_ += "					<div class='row'>";
	cad_ += "						<div class='col s4 m4 l3 xl2 hide_for_600px' data-col='c_s_" + pi_grid_id + "_filtros'>";
	cad_ += "							<div class='col_label'>Filtrar por</div>";
	cad_ += lc_html_table_select_ft;
	cad_ += "						</div>";
	cad_ += "						<div class='col s9 m6 l4 xl4'>";
	...
	cad_ += "						</div>";
	cad_ += "						<div class='col s3 m2 l5 xl6'>";
	cad_ += "							<div class='col_label'> </div>";
	cad_ += "							<div data-col='c_length_" + pi_grid_id + "'></div>";
	cad_ += "						</div>";
	cad_ += "					</div>";
	cad_ += "				</section>";
	cad_ += "				<div class='content_table' data-col='scroll_" + pi_grid_id + "' data-col_2='p-right-15'>";
	cad_ += "					<div class='row'>";
	cad_ += "						<div class='c_col col s12' data-col='c_" + pi_grid_id + "'></div>";
	cad_ += lc_html_table;
	cad_ += "					</div>";
	cad_ += "				</div>";
	cad_ += "				<div class='dataTables_wrapper' data-col='c_footer_" + pi_grid_id + "'></div>";
	cad_ += "			</div>";
	cad_ += "		</div>";
	...
	// Content begin
	cad_ += "	<div class='modal-content st_lks div_width_-15px lks_scroll_st_1 fixed_ft_1__1_row' data-col='scroll_" + pi_grid_id + "'>";
...
function fnc_create_table_data(pi_grid_id, pi_url, pi_grid_option, ib_table_fixed = false) {
	...
	*/
	...
	if (pi_grid_option === false) {
		...
		$('.container_table.footer_fixed .content_table table').each(function() {
			var pc_obj_id = $(this).attr('id');
			var po_nuevo_contenedor = document.querySelector('[data-col="c_footer_' + pc_obj_id + '"]');
			var po_object_paginate_mover = document.getElementById(pc_obj_id + '_paginate');
			var po_object_info_mover = document.getElementById(pc_obj_id + '_info');
			po_nuevo_contenedor.appendChild(po_object_info_mover);
			po_nuevo_contenedor.appendChild(po_object_paginate_mover);
		});
		...
		$(Obj_Col('', 'i_filter_' + pi_grid_id)).on('keyup change', function (event) {
			...
			fnc_tables_ejecutar_alternos(pi_grid_id);
			...
			if (event.which == 13) {
				if (fnc_Get_DataTable_RowCount(pi_grid_id, 'Filtered') > 0) {
					...
					fnc_ExecFunction_Grid(this, 'enter');
	...
	$('#' + pi_grid_id + ' tbody').on('click', 'td', function () {
		fnc_ExecFunction_Grid(this, 'click');
	});

	$('#' + pi_grid_id + ' tbody').on('dblclick', 'td', function () {
		fnc_ExecFunction_Grid(this, 'dblclick');
	});
...
function fnc_ExecFunction_Grid (io_grid, ic_event)
{
	var pc_table_id = $(io_grid).closest("table").attr('id');
	var pc_fnc = 'fnc_' + pc_table_id + '_row_click';
	
	if (ic_event === 'dblclick')
	{
		pc_fnc = 'fnc_' + pc_table_id + '_row_dblclick';
	}
	
	if ($.isFunction(window[pc_fnc]))
	{
		var po_table = $('#' + pc_table_id).DataTable();
		var row_data;

		if (ic_event === 'click' || ic_event === 'dblclick')
		{
			row_data = po_table.row($(io_grid).parents('tr')).data();	
		}
		else if (ic_event === 'enter')
		{
			row_data = po_table.row($('#' + pc_table_id + ' tbody tr.selected')).data();
		}

		eval(pc_fnc + ' (row_data)');
	}
};
		...
		if (obj_type_ === 'input') {
			...
			if ($(this).is(":text")) {
				...
				*/
				...
				var pb_value = false;
				if (pc_value === 'A' || parseInt(pc_value) === 1) {pb_value = true;}
				

				/*
				if (fc_ctip_frm === 'frm_ct') {
					if (pc_value === 'A') {
						pb_value = true;
					}
					else {
				...
				else if (fc_ctip_frm === 'frm_ts') {
					if (parseInt(pc_value) === 1) {
						pb_value = true;
					}
					else {
						pb_value = false;
				...
				*/
				fnc_checkbox_set_value(ic_form, data_col, pb_value);
			}
		}
			...
			if (data_col_tip === '_drt_') {
				...
				pc_value = row_data[data_col.substring(5, data_col.length)];
				pc_value = $(Obj_Col(ic_form, data_col, 'select') + (" option[value='" + pc_value + "']")).text();
			}
			else {
...
function fnc_table_export_excel() {
	...
	if (pc_s_col === 'col_v') {
		var pa_results = fnc_Array_Filtrar_por_columnas(ma_usp_Get_si_usu_ctl_conf, ['cform', 'cctrl'], [($('#' + pc_table_select_export).hasClass('generic') === true ? 'generic' : fc_ctip_frm), pc_table_select_export]);
		pa_results = fnc_Array_sortByKeyAsc(pa_results, 'nord');
		/*
		var pc_url = '/Home/lq_usp_Get_si_usu_ctl_conf?ic_ccod_emp=' + gc_ccod_emp + '&ic_ccod_usu=' + gc_ccod_usu + '&ic_cform=' + ($('#' + pc_table_select_export).hasClass('generic') === true ? 'generic' : fc_ctip_frm) + '&pi_cctrl=' + pc_table_select_export + '&pi_cft_group_cdsc=' + '&ic_load_BD=';
...
//fer33
function fnc_tables_ejecutar_alternos(ic_object = '') {
	ic_object = $.trim(ic_object);

	// Responsive
	if (ic_object.substring(0, 5) === 'grid_') {
		fnc_dataTable_recalc(ic_object);
	}
	else if (ic_object.substring(0, 4) === 'grid' && ic_object.length === 4) {
		fnc_dataTable_recalc();
	}
	/*
	else if (ic_object.substring(0, 4) === 'tab_') {
		fnc_dataTable_recalc();
	}
	*/
	else if (ic_object.substring(0, 4) === 'mdl_') {
		fnc_dataTable_recalc();
	}
	...
	*/
	...
	var pc_selector = '';
	if (ic_object.substring(0, 5) === 'grid_') {
		pc_selector = "[data-col='scroll_" + ic_object + "']";
	}
	else if (ic_object.substring(0, 4) === 'grid' && ic_object.length === 4) {
		//pc_selector = '#' + ic_object + ' [data-col^="scroll_grid_"]';
		pc_selector = '[data-col^="scroll_grid_"]';
	}
	else if (ic_object.substring(0, 4) === 'tab_') {
		pc_selector = '#' + ic_object + ', #' + ic_object + ' [data-col^="scroll_"]';
	}
	else if (ic_object.substring(0, 4) === 'mdl_') {
		pc_selector = '#' + ic_object + ' .modal-content, #' + ic_object + ' .modal-content [data-col^="scroll_"]';
	}
	else if (ic_object.substring(0, 2) === 'mn') {
		// pc_selector = '#' + ic_object + ' [data-col^="scroll_"]';
		pc_selector = '[data-col^="scroll_mn"]';
	}
	//alert(pc_selector);
	if (pc_selector !== '') {
		$(pc_selector).each(function() {
			var ac_object = fnc_GET_Obj_Attributes(this);
			fnc_scroll__add_class(ac_object.form, ac_object.col, ac_object.col_2);
		});
	}
};

function fnc_scroll__add_class (ic_form, ic_object, ic_class)
{
	//if (ic_object === 'scroll_') {return;};
	
	if ($(Obj_Col(ic_form, ic_object)).exists()) {
		if (fnc_GET_Obj_isScrollable(ic_form, ic_object)[0].SV === true) {
			$(Obj_Col(ic_form, ic_object)).addClass(ic_class);
		}
		else {
			$(Obj_Col(ic_form, ic_object)).removeClass(ic_class);
		}
	}
};

function fnc_GET_Obj_isScrollable (ic_form, ic_object)
{
	var po_obj = document.querySelector(Obj_Col(ic_form, ic_object));

	// Vertical
	var y1 = po_obj.scrollTop;  
	po_obj.scrollTop += 1; 
	var y2 = po_obj.scrollTop; 
	po_obj.scrollTop -= 1; 
	var y3 = po_obj.scrollTop; 
	po_obj.scrollTop = y1; 

	// Horizontal
	var x1 = po_obj.scrollLeft;  
	po_obj.scrollLeft += 1; 
	var x2 = po_obj.scrollLeft; 
	po_obj.scrollLeft -= 1; 
	var x3 = po_obj.scrollLeft; 
	po_obj.scrollLeft = x1;
...
function fnc_dataTable_recalc(ic_grid = '') {
...
function fnc_tables_cargar_caracteristicas() {
    ...
    $('table tbody').on('click', 'td.dtr-control', function () {
		fnc_tables_ejecutar_alternos($(this).closest('table').attr('id'));
    });
	...
	$("#btn_p_row").click(function () {
		...
		fnc_tables_ejecutar_alternos('grid');
	});
	...
	$("[data-col^='c_length_']").each(function() {
		var ac_object = fnc_GET_Obj_Attributes(this);
		var pc_grid = ac_object.col.replace('c_length_', '');
		
		//if ($(Obj_Col(ac_object.form, ac_object.col)).exists()) {
			$(Obj_Col(ac_object.form, ac_object.col)).prepend(fnc_html_select__grid_length(pc_grid));
		//}
	});
	
	$("[data-col^='c_length_'] > select").change(function() {
		// Longitud
		var ac_object = fnc_GET_Obj_Attributes(this);
		pc_select_col = ac_object.col;
		$("select[name='" + pc_select_col + "']").val($(this).find(":selected").val()).trigger('change');
		// Comportamiento
		pc_select_col = pc_select_col.replace('_length', '');
		fnc_tables_ejecutar_alternos(pc_select_col);
	});
	...
	$('table').each(function() {
		...
		$('#' + obj_id_).on('key.dt', function(e, datatable, key, cell, originalEvent) {
			if (key === 13) {
				...
				// Terminar y probar con el formulario factura
				//$(Obj_Col(fc_frm_cb, 'ccod_aac')).val();
				var button_obj = Obj_Col('', '_lnk_add_' + obj_id_, 'button' , 'lnk');
				
				if (!fnc_ClickButton(button_obj)) {
					var funcion_ = 'fnc_add_' + obj_id_;
					fnc_ExecFunction(funcion_);
				}
				
				/*
				if ($('button[data-lnk="_lnk_add_' + obj_id_ + '"]').exists()) {
...
/*
function fnc_prepend (ic_selector, ic_html) {
	if ($(ic_selector).exists()) {
		$(ic_selector).prepend(ic_html);
	}
};
*/
function fnc_html_select__grid_length(ic_grid) {
	var pa_object = fnc_GET_Obj_Attributes($('#' + ic_grid));
	var pc_modal = pa_object.modal_container;
	var pc_data_zi = (pc_modal === "" ? "" : " data-zi='" + pc_modal + "'");

	return "\
	<select class='select2 s2'" + pc_data_zi + " data-allow-clear='false' data-col='" + ic_grid + "_length'> \
...
/*
// 13/11/2020: Se esta comentado por que no se usa creo.
function f_grid_complementos(grid_id) {
    ...
    $(Obj_Col('', 'i_filter_' + grid_id)).on('keyup change', function (event) {
		...
		fnc_tables_ejecutar_alternos('', grid_id);
														...
														if (ic_grid_select !== '') {
															...
															if (!pt_table_select.data().count()) {
																...
																if (pt_table.rows().any()) {
																	...
																	fnc_tables_ejecutar_alternos(ic_grid_id);
																}
															}
														}
														else {
...
function fnc_DataTable_fnAddData_to_array (ic_grid, ia_source)
{
	...
	fnc_tables_ejecutar_alternos(ic_grid);
};
...
// Estas dos funciones mover al grupo de array
...
function fnc_SET_text__to__column_header__DataTable (ic_grid, in_col, ic_text)
{
    var oTable = $('#' + ic_grid).DataTable();
	//var pc_colum_text = $(oTable.column(in_col).header()).text();
	$(oTable.column(in_col).header()).text(ic_text);
	$(oTable.column(in_col).footer()).text(ic_text);
};
